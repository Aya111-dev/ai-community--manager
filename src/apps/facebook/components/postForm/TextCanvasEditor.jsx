...
// ─── Canvas ───────────────────────────────────────────────────────────────────
export function StoryCanvas({ styleSettings, updateStyle, mediaUrl, mediaKind, onMediaUpload, editing, setEditing, textDivRef, fileRef, onStoryDescriptionChange }) {
  ...
  useEffect(() => {
    ...
    sel?.removeAllRanges()
    sel?.addRange(range)
  }, [editing, textDivRef, styleSettings.textCanvasText])
            ...
            {editing ? (
              ...
              <div
                style={{
                  color: tc.color, fontFamily: tc.fontFamily, fontSize: tc.fontSize, fontWeight: tc.fontWeight,
                  textAlign: tc.align, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                  maxWidth: 130, minWidth: 24, padding: '2px 5px', borderRadius: 4,
                  textShadow: '0 1px 4px rgba(0,0,0,0.5)', direction: 'ltr',
                }}
              >
                {tc.text}
              </div>
...
export function StoryControls({
  ...
  const handleDescriptionChange = (value) => {
    onStoryDescriptionChange?.(value)
    updateStyle({
      textCanvasEnabled: true,
      textCanvasText: value,
      textCanvasY: styleSettings.textCanvasY ?? 82,
      textCanvasX: styleSettings.textCanvasX ?? 50,
      textCanvasFontSize: styleSettings.textCanvasFontSize ?? 18,
    })
  }

  const handleOpenDescription = () => {
    onToggleDescriptionPanel?.(true)
    onAddText?.()
    if (!storyDescription.trim()) {
      handleDescriptionChange('Votre description')
    } else {
      handleDescriptionChange(storyDescription)
    }
    ...
    <div style={{
      ...
      {tc.enabled ? (
        ...
        <button
          ...
          style={{
            color: descriptionPanelOpen ? '#fff' : '#a78bfa',
            background: descriptionPanelOpen ? 'rgba(167,139,250,0.28)' : 'rgba(167,139,250,0.12)',
            padding: '10px 0',
            borderRadius: 10,
            display: 'flex',
            justifyContent: 'center',
            border: descriptionPanelOpen ? '1px solid rgba(167,139,250,0.45)' : '1px solid rgba(167,139,250,0.2)',
            width: '100%',
            fontWeight: 800,
            fontSize: 14,
          }}
...
export function TextCanvasEditor({
  ...
  storyDescription, onStoryDescriptionChange, onRegenerateDescription,
  isGenerating, descriptionPanelOpen, onToggleDescriptionPanel,
}) {
