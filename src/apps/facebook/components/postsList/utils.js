...
export function getMediaOverlayData(post) {
  const style = post.style ?? {}
  return {
    overlayText: typeof style.overlayText === 'string' ? style.overlayText.trim() : (typeof post.text === 'string' ? post.text.trim() : ''),
    overlayTextX: Math.max(0, Math.min(100, style.overlayTextX ?? post.textPosition?.x ?? 50)),
    overlayTextY: Math.max(0, Math.min(100, style.overlayTextY ?? post.textPosition?.y ?? 84)),
    overlayTextFontFamily: style.overlayTextFontFamily ?? style.fontFamily ?? '-apple-system, Helvetica, Arial, sans-serif',
    overlayTextColor: style.overlayTextColor ?? '#ffffff',
  }
}
