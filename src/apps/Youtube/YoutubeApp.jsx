...
function useResolvedPostDuration(post) {
  ...
  useEffect(() => {
    const isVideoMedia = !!post?.mediaUrl && post?.mediaType?.startsWith('video/');
...
function YoutubeCreateLanding({ platformMeta, onBack, onPickType, publishedPosts, onSelect, onEditPost }) {
  const [publishedDeviceView, setPublishedDeviceView] = React.useState('Mobile');
...
function YoutubeCreatePostPage({
  ...
  const [media, setMedia] = React.useState(() => {
    ...
    const inferredType = initialPost?.mediaType ?? (initialPost?.mediaUrl?.startsWith?.('http') ? 'image/*' : 'image/*');
    return [
      {
  ...
  const getAutoMediaForType = (typeId) => {
    if (typeId === 'shorts') {
      return {
        id: `ai-short-${Date.now()}`,
        file: null,
        url: 'https://picsum.photos/id/1011/900/1600',
        size: 0,
        type: 'image/jpeg',
        duration: '0:19',
      };
  ...
  const generateWithAI = () => {
    ...
    try {
      ...
      const titleGenerated = buildTitle(subject, postType);
      const captionGenerated = buildCaption(subject, postType, keywords);
      const hashtagsGenerated = buildHashtags(subject, postType, keywords);

      setTitle(titleGenerated);
      setDescription(captionGenerated);
      setHashtags(hashtagsGenerated);

      setDuration(pickDefaultDuration(postType));

      // Media: if user didn't choose a file yet, we auto-fill a demo media
      if (!media || media.length === 0) {
        const auto = getAutoMediaForType(postType);
        setMedia([auto]);
      }
    } finally {
    ...
    <section className="posts-card card animation-slide-up">
      ...
      <div style={{ display: 'grid', gap: 16, marginTop: 16 }}>
        <section className="card shadowless" style={{ padding: 18 }}>
          ...
          <div className="card shadowless" style={{ padding: 18, borderRadius: 20, marginBottom: 18, border: '1px solid rgba(109, 40, 217, 0.12)'}}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ marginLeft: 'auto', color: 'var(--text-light)', fontSize: 14 }}>
                {typeLabel}
              </div>
            </div>
            ...
            <section className="card shadowless" style={{ padding: 16, borderRadius: 18, background: 'rgba(248, 250, 252, 0.95)', marginBottom: 18 }}>
              ...
              <div style={{ 
                position: 'relative', 
                width: '100%', 
                maxWidth: postType === 'shorts' ? 280 : '100%',
                margin: '0 auto',
                aspectRatio: postType === 'shorts' ? '9 / 16' : '16 / 9', 
                borderRadius: 20, 
                overflow: 'hidden', 
            ...
            {deviceView === 'Mobile' ? (
              <div className="yt-phone-frame">
                <YoutubeHomeFeed
                  ...
                  allowFallback={activeTab === 'Publiés'}
