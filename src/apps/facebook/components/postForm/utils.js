...
export function buildInitialMedia(cfg, initialMedia = []) {
  const mediaFiles = Array(cfg?.mediaCount || 1).fill(null)
  const extraMedia = []
  if (Array.isArray(initialMedia)) {
    initialMedia.forEach((m, i) => {
      if (i < mediaFiles.length) mediaFiles[i] = m
      else extraMedia.push(m)
    })
  }
  return { mediaFiles, extraMedia }
}
...
export function isVertical(postType) {
  return ['story', 'reel'].includes(postType)
}
...
export function getPostStyle(post, compact, isMobile, expanded = false) {
  ...
  return {
    container: {
      background: '#fff', borderRadius: 0, border: 'none',
      width: '100%', maxWidth: expanded ? 860 : '100%',
      margin: '0 auto', overflow: 'hidden',
    },
    text: {
      fontFamily: style.fontFamily ?? '-apple-system, Helvetica, Arial, sans-serif',
      fontSize: 15, textAlign: 'left', color: '#111827',
    },
...
export function getMediaOverlayData(post) {
  ...
  return {
    ...
    // ── NEW: expose font size, bold, align so the viewer renders correctly ──
...
export function getPrimaryMedia(post) {
  ...
  return { src, type, presentation: getMediaPresentation(firstMedia ?? post, post.style) }
}
...
export function getStoryBackground(style = {}) {
  ...
  // fallback to the first gradient used in TextCanvasEditor
  return 'linear-gradient(145deg,#c026d3,#e11d48)'
}
...
export function getPostComments(post) {
  ...
  return post.comments
    .map((comment, index) => {
      const authorName = comment.author ?? comment.authorName ?? comment.user?.name ?? comment.username ?? 'Facebook User'
