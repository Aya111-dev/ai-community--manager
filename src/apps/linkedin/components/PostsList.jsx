...
import { TABS, DEVICE_VIEWS, LI_BG, CARD_BORDER, TEXT, SUBTLE } from './postsList/constants'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', width: isSmallScreen ? '100%' : 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', borderRadius: 999, background: '#f1f5f9', padding: '4px', gap: 2, flexWrap: 'wrap', width: isSmallScreen ? '100%' : 'auto' }}>
            {TABS.map(tab => (
              <button key={tab} type="button" onClick={() => setActiveTab(tab)}
                style={{
                  cursor: 'pointer', borderRadius: 999, padding: '7px 16px', fontSize: 13, fontWeight: 600,
                  border: 'none', transition: 'all 0.15s', flex: isSmallScreen ? 1 : '0 0 auto',
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', width: isSmallScreen ? '100%' : 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', borderRadius: 999, background: '#f1f5f9', padding: '4px', gap: 2, width: isSmallScreen ? '100%' : 'auto' }}>
