      ...
      <div style={{ background: '#fff' }}>
        {media.map((item, index) => (
          <div key={`${post.id}-mobile-preview-${index}`} style={{ borderTop: index === 0 ? 'none' : `6px solid ${FB_BG}` }}>
            ...
            <div style={{ padding: '8px 12px 6px', fontSize: 13, color: FB_SECONDARY, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0, whiteSpace: 'nowrap' }}>
                ...
                <span style={{ whiteSpace: 'nowrap' }}>{Math.max(1, Math.round(engagement.likes / media.length))}</span>
              </div>
              <div style={{ display: 'flex', gap: 10, flexShrink: 0, whiteSpace: 'nowrap' }}>
                <span style={{ whiteSpace: 'nowrap' }}>{formatMetricLabel(Math.max(1, Math.round(engagement.comments / media.length)), 'comment')}</span>
                <span style={{ whiteSpace: 'nowrap' }}>{formatMetricLabel(Math.max(1, Math.round(engagement.shares / media.length)), 'share')}</span>
              </div>
            </div>
