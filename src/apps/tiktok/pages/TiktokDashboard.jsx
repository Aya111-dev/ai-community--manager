...
export default function TiktokDashboard() {
  ...
  const createStoryPayload = () => {
    ...
    return {
      ...
      contentPreview: storyMediaPreview || storyCover,
  ...
  const handleStoryPublish = () => {
    ...
    if (editingPublishedFromCreateId) {
      setTiktokPosts((prev) => prev.map((item) => (item.id === editingPublishedFromCreateId ? { ...item, ...payload, status: 'published' } : item)));
  ...
  const handleAiSubmit = (event) => {
    ...
    if (!prompt) return;
    const now = new Date();
    const mediaKind = creationKind === 'video' ? 'video' : creationKind === 'image' ? 'image' : 'story';
    const { caption, hashtags } = generateMetadataFromPrompt(prompt, mediaKind);
    ...
    setStoryCaption(caption);
    setStoryHashtags(hashtags);
    setStoryMediaPreview(generatedPreview);
    if (generatedPreview) setStoryCover(generatedPreview);
  ...
  const handlePhotoAiSubmit = (event) => {
    event.preventDefault();
    const prompt = photoPrompt.trim();
    if (!prompt) return;
    const now = new Date();
    setPhotoPreview(prompt);
    const { caption, hashtags } = generateMetadataFromPrompt(prompt, 'photo');
    setPhotoCaption(caption);
    setPhotoHashtags(hashtags);
  ...
  const handlePhotoPublish = () => {
    ...
    const payload = {
      ...
      sound: selectedSound,
      status: 'published',
      // Métadonnées pour l'aperçu TikTok - utilise les hashtags et captions générés si disponibles
    ...
    if (editingPublishedFromCreateId) {
      setTiktokPosts((prev) => prev.map((item) => (item.id === editingPublishedFromCreateId ? { ...item, ...payload } : item)));
      setPublishStatus('Modifications sauvegardées et publiées !');
      setEditingPublishedFromCreateId(null);
    } else {
  ...
  const handleVideoAiSubmit = (event) => {
    event.preventDefault();
    const prompt = videoPrompt.trim();
    if (!prompt) return;
    const now = new Date();
    setVideoPreview(prompt);
    const { caption, hashtags } = generateMetadataFromPrompt(prompt, 'video');
    setVideoCaption(caption);
    setVideoHashtags(hashtags);
  ...
  const handleNextPhoto = () => {
    if (manualMediaFiles.length > 0) {
      setCurrentPhotoIndex((prevIndex) => {
        ...
        // Mettre à jour l'aperçu avec la nouvelle photo en créant une URL
        setPhotoPreview(URL.createObjectURL(manualMediaFiles[nextIndex]));
        return nextIndex;
      });
    }
  };
  ...
  const handlePreviousPhoto = () => {
    if (manualMediaFiles.length > 0) {
      setCurrentPhotoIndex((currentIndex) => {
        ...
        // Mettre à jour l'aperçu avec la nouvelle photo en créant une URL
        setPhotoPreview(URL.createObjectURL(manualMediaFiles[prevIndex]));
        return prevIndex;
      });
    }
  };
...
const handleSchedulePublish = () => {
  ...
  const handleDeleteScheduledPost = (postId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce contenu planifié ?')) {
      setScheduledItems(prev => prev.filter(item => item.id !== postId));
      setShowScheduledMoreOptions(null);
      if (editingScheduled === postId || showEditOptions === postId) {
        setEditingScheduled(null);
        setEditedScheduledData({});
        setShowEditOptions(null);
      }
  ...
  const handleDeleteDraftPost = (postId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce brouillon ?')) {
      setDraftItems((prev) => prev.filter((item) => item.id !== postId));
  ...
  const handleEditPublishedPost = (post) => {
    ...
    const tagText = typeof post.hashtags === 'string' ? post.hashtags : '';
  ...
  const handleDeletePublishedPost = (postId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce contenu publié ?')) {
      // Mettre à jour l'état tiktokPosts en supprimant le post
      setTiktokPosts(prev => prev.filter(post => post.id !== postId));
          ...
          {selectedPublishType === null && (
            <div className="create-content-modal">
              ...
              <div className="create-modal-buttons">
                ...
                >
                  <div className="modal-button-icon" style={{
                    flexShrink: 0
                  }}>
                    <Video size={24} />
                  </div>
                  <span className="modal-button-text">Video</span>
                </button>
                    ...
                    <div className="tiktok-phone-frame">
                      <div className="tiktok-story-container">
                        ...
                        <div className="story-media-container">
                          {manualMediaFiles[currentPhotoIndex]?.type?.startsWith('video/') ? (
                            <video
                              ...
                              playsInline
                            >
                              <source src={URL.createObjectURL(manualMediaFiles[currentPhotoIndex])} />
                            </video>
                          ) : (
                            <img
                              className="story-preview-image"
                              src={URL.createObjectURL(manualMediaFiles[currentPhotoIndex])}
                              alt="Image preview"
                            />
                          )}
                          ...
                          <div className="story-overlay">
                            ...
                            <div className="story-footer">
                              ...
                              {/* Flèches de navigation pour les photos */}
                              <div className="photo-navigation">
                                ...
                                >
                                  <ChevronLeft size={24} fill="white" />
                                </button>
                                <button 
                ...
                <div className="ai-interface-exact">
                  ...
                  <div className="publish-ai-form">
                    ...
                    <button
                      ...
                      disabled={!videoPrompt.trim() || videoPrompt.length < 3}
                    >
                      <Sparkles size={16} />
                      <span>Générer</span>
                    </button>
              ...
              <div className="ai-interface-exact">
                ...
                <div className="publish-ai-form">
                  ...
                  <button
                    ...
                    disabled={!photoPrompt.trim() || photoPrompt.length < 3}
                  >
                    <Sparkles size={16} />
                    <span>Générer</span>
                  </button>
              ...
              <div className="ai-interface-exact">
                ...
                <div className="publish-ai-form">
                  ...
                  <button
                    ...
                    disabled={!aiPrompt.trim() || aiPrompt.length < 3}
                  >
                    <Sparkles size={16} />
                    <span>Générer</span>
                  </button>
                            ...
                            {creationKind === 'video' ? (
                              ...
                              <div className="ai-generated-story">
                                <div className="ai-story-content">
                                  ...
                                  <div className="ai-story-overlay">
                                    <div className="ai-story-effects">
                                      {storyMode === 'trendy' && <span className="effect-badge">🔥 Trending</span>}
                                      {storyMode === 'minimal' && <span className="effect-badge">✨ Minimal</span>}
                                      {storyMode === 'vibrant' && <span className="effect-badge">🌈 Vibrant</span>}
                                      {storyMode === 'professional' && <span className="effect-badge">💼 Pro</span>}
                                    </div>
                                  </div>
                                </div>
                              </div>
                ...
                <div className="ai-story-preview" style={{ marginTop: 18 }}>
                  <div className="ai-tiktok-preview">
                    <div className="tiktok-phone-frame">
                      <div className="tiktok-story-container">
                        <div className="story-media-container">
                          {manualMediaFiles[0].type?.startsWith('video/') ? (
                            <video className="story-preview-video" autoPlay muted loop playsInline>
                              <source src={URL.createObjectURL(manualMediaFiles[0])} />
                            </video>
                          ) : (
                            <img
                              className="story-preview-image"
                              src={URL.createObjectURL(manualMediaFiles[0])}
                              alt="Story preview"
                            />
                          )}
                          <div className="story-overlay">
                            ...
                            <div className="story-caption">
                              {creationKind === 'story' ? (
                                <p>{storyPreview || "Ta story s'affiche ici"}</p>
                              ) : (
                                <>
          ...
          {filteredPosts.length === 0 ? (
            ...
            <div className={`posts-grid posts-grid-${viewMode}`}>
              {filteredPosts.map((post) => {
                ...
                const isBookmarked = interactions.bookmarked || post.bookmarked || false;
                const isPlaying = playingVideos.has(post.id);
                const currentLikes = interactions.likes || post.likes || 0;
                const currentComments = interactions.comments || post.comments || 0;
                const currentShares = interactions.shares || post.shares || 0;

                return (
                  <div key={post.id} className={`tiktok-post-preview ${viewMode}`}>
                    {/* Conteneur principal TikTok */}
                    <div className="tiktok-video-container">
                      {/* Vidéo ou image */}
                      {post.type === 'video' || post.contentPreview?.includes('.mp4') ? (
                        <video 
                          id={`tiktok-video-${post.id}`}
                          className="tiktok-video-player"
                          autoPlay={viewMode === 'desktop' && !isPlaying}
                          muted={true}
                          loop={true}
                          playsInline={true}
                          poster="https://picsum.photos/400/800?blur=2"
                          onClick={() => handlePlayPauseVideo(post.id)}
                        >
                      ...
                      <div className="tiktok-video-overlay">
                        ...
                        <div className="tiktok-user-section">
                          <div className="tiktok-user-info-row">
                            <div className="tiktok-avatar-container">
                              <div 
                                className="tiktok-avatar-circle"
                            ...
                            <div className="tiktok-user-details">
                              ...
                              {(activeTab === 'draft' ||
                                (post.type !== 'story' && post.contentType !== 'story')) && (
                                <>
                          ...
                          <div 
                            className="tiktok-music-info"
                            onClick={() => handleOpenMusic(post.sound)}
                            style={{ cursor: 'pointer' }}
                          >
                            <div className="music-icon">🎵</div>
                            <div className="music-text">
                              <div className="music-title">{post.sound || 'Son original'}</div>
                              {post.soundIcon && <span className="music-emoji">{post.soundIcon}</span>}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="tiktok-side-actions">
                          <div className="side-action">
                            ...
                            >
                              <Heart size={28} fill={isLiked ? '#ff0050' : 'none'} color={isLiked ? '#ff0050' : 'white'} />
                            </button>
                            <span className="action-count">{formatCount(currentLikes)}</span>
                          </div>
                          <div className="side-action">
                            <button 
                            ...
                            <span className="action-count">{formatCount(currentComments)}</span>
                          </div>
                          <div className="side-action">
                            <button 
                              className="action-button"
                            ...
                            <span className="action-count">{formatCount(currentShares)}</span>
                          </div>
                          <div className="side-action">
                            <button 
                            ...
                            >
                              <Bookmark size={28} fill={isBookmarked ? 'white' : 'none'} />
                            </button>
                          </div>
                          {post.duration && (
                        ...
                        {/* Badge planifié pour contenu planifié */}
                        {activeTab === 'scheduled' && (
                          <div className="scheduled-badge">
                            ...
                            {post.scheduledAt && (
                              <span className="scheduled-time">
                                {formatSelectedDate(post.scheduledAt)}
                              </span>
                            )}
                          </div>
                        )}

                        {/* Options d'édition pour posts planifiés */}
                        {activeTab === 'scheduled' && showEditOptions === post.id && (
                          <div className="edit-options-panel">
                            ...
                            <div className="edit-options-content">
                              <div className="edit-option">
                                ...
                                <input
                                  type="datetime-local"
                                  value={editedScheduledData.scheduledAt ? 
                                    new Date(editedScheduledData.scheduledAt).toISOString().slice(0, 16) : ''}
                                  onChange={(e) => setEditedScheduledData(prev => ({
                                    ...prev,
                              ...
                              <div className="edit-option">
                                <label>Son</label>
                                <select
                                  value={editedScheduledData.sound || ''}
                                  onChange={(e) => setEditedScheduledData(prev => ({
                                    ...prev,
                                ...
                                >
                                  ...
                                  {soundCategories.flatMap(category => 
                              ...
                              <div className="edit-option">
                                <label>Légende</label>
                                <textarea
                                  value={editedScheduledData.description || ''}
                                  onChange={(e) => setEditedScheduledData(prev => ({
                                    ...prev,
                            ...
                            <div className="edit-options-actions">
                              <button 
                                ...
                                onClick={() => handleSaveScheduledChanges(post.id)}
                              >
                                <Check size={16} />
                                Sauvegarder
                              </button>
                        ...
                        {/* Menu plus d'options - uniquement pour les posts publiés */}
                        {activeTab === 'published' && (
                          <div className="more-options">
                            ...
                            {showPublishedEditOptions === post.id && (
                              <div className="published-edit-dropdown">
                                <button type="button" onClick={() => handleEditPublishedPost(post)}>
                                  Modifier
                                </button>
                                <button type="button" onClick={() => handleDeletePublishedPost(post.id)}>
                                  Supprimer
                                </button>
                              </div>
                            )}
                          ...
                          <div className="more-options">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                setShowPublishedEditOptions(null);
                                setShowScheduledMoreOptions(null);
                                setShowDraftEditOptions(showDraftEditOptions === post.id ? null : post.id);
                              }}
                              title="Plus d'options"
                            >
                              <MoreVertical size={20} color="white" />
                            </button>
                            {showDraftEditOptions === post.id && (
                              <div className="published-edit-dropdown">
                                <button
                                  type="button"
                                  onClick={(e) => {
                          ...
                          <div className="more-options">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                setShowDraftEditOptions(null);
                                setShowPublishedEditOptions(null);
                                setShowScheduledMoreOptions(
                                  showScheduledMoreOptions === post.id ? null : post.id
                                );
                              }}
                              title="Plus d'options"
                            >
                            ...
                            {showScheduledMoreOptions === post.id && (
                              <div className="published-edit-dropdown">
                                <button
                                  type="button"
                                  onClick={(e) => {
                        ...
                        {/* Panneau d'édition pour les posts publiés */}
                        {activeTab === 'published' && editingPublished === post.id && (
                          <div className="published-edit-panel">
                            <div className="published-edit-header">
                              <h4>Modifier le contenu publié</h4>
                              <button 
                                className="close-edit-btn"
                            ...
                            <div className="published-edit-content">
                              <div className="edit-option">
                                <label>Légende</label>
                                <textarea
                              ...
                              <div className="edit-option">
                                <label>Statut du boost</label>
                                <div className="boost-toggle">
                                  <input
                                    type="checkbox"
                                    checked={editedPublishedData.boostEnabled || false}
                                    onChange={(e) => setEditedPublishedData(prev => ({
                                      ...prev,
                                      boostEnabled: e.target.checked
                                    }))}
                                  />
                                  <span>Activer le boost pour augmenter la visibilité</span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="published-edit-actions">
                              <button 
                                className="save-edit-btn"
                                onClick={() => handleSavePublishedChanges(post.id)}
                              >
                                <Check size={16} />
                                Sauvegarder
                              </button>
