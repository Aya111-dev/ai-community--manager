...
function ThreadFeedCard({ post, viewMode }) {
  if (isThreadsSocialPost(post)) {
    return (
      <article className={`card feed-card threads-feed-card threads-feed-card-social ${viewMode}`}>
        <ThreadsSingleVideoPost post={post} viewMode={viewMode} className="threads-feed-social-post" />
      </article>
    );
  }
