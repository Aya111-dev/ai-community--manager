...
function XCreatePost({ onCancel, onPublish, onSchedule, onSaveDraft, initialPost }) {
  ...
  const fillFromAI = () => {
    ...
    const typeTone = {
      text: 'tweet percutant',
      image: 'post visuel engageant',
      video: 'video courte qui retient l attention',
      thread: 'thread clair et structure'
    }[selectedType];

    setCaption(`✨ ${promptText}\n\n${typeTone} pour X avec un ton moderne et naturel.`);
    if (selectedType === 'thread') {
      setThreadPosts([
        { text: `${promptText} : contexte rapide`, hashtags: '#x #thread' },
        { text: 'Point cle a retenir pour ton audience', hashtags: '#conseil #community' },
        { text: 'Conclusion avec appel a l action', hashtags: '#cta #socialmedia' }
      ]);
    }
    setHashtags(`#x #socialmedia #ai #contenu #${promptText.split(' ')[0]?.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() || 'trend'}`);
    setMedia((prev) => {
      if (prev.length || activeConfig.maxMedia === 0) return prev;
      if (selectedType === 'video') {
        return [{ id: `x-video-ai-${Date.now()}`, url: fakeVideoPool[0], kind: 'video' }];
      }
      if (selectedType === 'image') {
        return Array.from({ length: activeConfig.maxMedia }).map((_, index) => ({
          id: `x-image-ai-${Date.now()}-${index}`,
          url: fakeImagePool[index % fakeImagePool.length],
          kind: 'image'
        }));
      }
      return prev;
  ...
  const generateCaption = () => {
    const promptText = aiPrompt.trim() || 'Nouvelle idee';
    if (selectedType === 'thread') {
      setThreadPosts([
        { text: `${promptText} - intro`, hashtags: '#x #thread' },
        { text: 'Ce que ton audience doit retenir', hashtags: '#tips' },
        { text: 'Conclusion avec appel a l action', hashtags: '#cta' }
      ]);
      return;
    }
    setCaption(`✨ ${promptText}\n\nUn message pense pour performer sur X et lancer la conversation.`);
  };

  const generateHashtags = () => {
    setHashtags('#x #community #marketing #ai #content');
  };
  ...
  const generateMedia = () => {
    ...
    setMedia((prev) => {
      if (prev.length >= activeConfig.maxMedia) return prev;
      const isVideo = activeConfig.aiKind === 'video';
      const sourcePool = isVideo ? fakeVideoPool : fakeImagePool;
      const remaining = activeConfig.maxMedia - prev.length;
      const generatedItems = Array.from({ length: remaining }).map((_, index) => {
        const sourceIndex = (prev.length + index) % sourcePool.length;
        return {
          id: `${selectedType}-ai-${Date.now()}-${prev.length + index}`,
          url: sourcePool[sourceIndex],
          kind: isVideo ? 'video' : 'image'
        };
      });
      return [...prev, ...generatedItems];
    });
    setError('');
  };
          ...
          <div className="x-preview-card">
            ...
            <div className="x-editor-block">
              <div className="x-editor-label-row">
                <span>HASHTAGS</span>
                <div className="x-editor-icons">
                  <button type="button" className="x-icon-btn" aria-label="Generation IA hashtags" onClick={generateHashtags}>
                    <Sparkles size={14} />
                  </button>
                  <button
                    type="button"
            ...
            {activeConfig.maxMedia > 0 ? (
              <div className="x-editor-block">
                <div className="x-editor-label-row">
                  <span>MEDIA</span>
                  <div className="x-editor-icons">
                    <button type="button" className="x-icon-btn" aria-label="Generation IA media" onClick={generateMedia}>
                      <Sparkles size={14} />
                    </button>
                  </div>
                </div>
