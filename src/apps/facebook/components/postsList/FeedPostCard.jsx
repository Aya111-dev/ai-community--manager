...
export function FeedPostCard({ post, deviceView, compact = false, expanded = false, onEditPost, onDeletePost, onOpen }) {
  ...
  return (
    <div onClick={canOpen ? e => onOpen(post, e) : undefined} style={{ ...styles.container, cursor: canOpen ? 'pointer' : 'default', background: '#fff' }}>
      ...
      {/* Reaction count bar */}
      {post.status !== 'draft' && (
        <div style={{ padding: '8px 12px', fontSize: 13, color: FB_SECONDARY, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 13, color: FB_SECONDARY }}>{formatCount(engagement.likes)}</span>
          </div>
          <div style={{ display: 'flex', gap: 10, fontSize: 13, color: FB_SECONDARY }}>
