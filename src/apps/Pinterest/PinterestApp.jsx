...
import { pinToApiPayload, apiPostToPin, resolveMediaUrl } from '../../services/postPersistence.js';
import {
...
export default function PinterestApp({ onBack }) {
  ...
  const handleSave = async (updated) => {
    try {
      let apiPayload = pinToApiPayload(updated);
      ...
      const existingId = updated._id || updated.id;
      const isNewLocalId = String(existingId || '').startsWith('pin-');
      const saved = !isNewLocalId && existingId
        ? await updatePost(existingId, apiPayload)
        : await createPost(apiPayload);
      const mapped = apiPostToPin(saved);
      setPins(prev => {
        const idx = prev.findIndex(p => p.id === mapped.id || p._id === mapped._id);
        if (idx === -1) return [{ ...mapped, likes: mapped.likes ?? 0, comments: mapped.comments ?? 0, saves: mapped.saves ?? 0, views: mapped.views ?? 0 }, ...prev];
        const next = [...prev];
        next[idx] = { ...prev[idx], ...mapped };
        return next;
      });
    } catch (error) {
      alert(error.message || 'Impossible d\'enregistrer l\'épingle');
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (id && !String(id).startsWith('pin-')) {
        await deletePost(id);
      }
    } catch {
  ...
  useEffect(() => {
    getPosts({ platform: 'pinterest' })
      .then((loaded) => {
        if (!Array.isArray(loaded) || loaded.length === 0) return;
        const fromApi = loaded.map(apiPostToPin);
        setPins((prev) => {
          const apiIds = new Set(fromApi.map((p) => p.id));
          const localOnly = prev.filter((p) => !apiIds.has(p.id) && String(p.id).startsWith('pin-'));
  ...
  return (
    <main className={`pn-screen ${deviceView === 'Mobile' ? 'is-mobile' : 'is-desktop'}`}>
      ...
      <header className="pn-header">
        ...
        <div className="pn-header-center">
          <div className="pn-search-bar">
            ...
            <input
              ...
              placeholder="Rechercher une épingle…"
            />
          </div>
        </div>
        )}
        ...
        <div className="pn-header-right">
          ...
          {isMobile ? (
            <>
              <button
                type="button"
                className="pn-icon-btn-square pn-icon-btn-square--primary"
                aria-label="Créer"
                onClick={() => setModal({ type: 'create-picker' })}
              >
                <Plus size={18} />
                <span className="pn-icon-btn-square-label">Créer</span>
              </button>
            </>
          ) : (
            <>
        ...
        {isMobile && (
          <div className="pn-header-center" aria-label="Rechercher">
            <div className="pn-search-bar">
              <Search size={14} />
              <input
                ref={searchInputRef}
          ...
          {!selectedBoardName ? (
            <>
              ...
              <div className="pn-filter-row">
                {isMobile ? (
                  <>
                    <div className="pn-filter-mobile-top">
                      <button type="button" className="pn-btn pn-btn-ghost pn-btn-sm pn-filter-strategy">
                        Stratégie
                      </button>
                      <div className="pn-device-toggle" aria-label="Vue appareil">
                        {['Desktop', 'Mobile'].map(d => (
                          <button
                            key={d}
                            type="button"
                            className={deviceView === d ? 'active' : ''}
                    ...
                    <div className="pn-filter-mobile-filters">
                    ...
                    <div className="pn-filter-mobile-tableau">
                  ...
                  <>
                    ...
                    <div className="pn-device-toggle pn-device-toggle-inline" aria-label="Vue appareil">
                      {['Desktop', 'Mobile'].map(d => (
                        <button
                          key={d}
                          type="button"
