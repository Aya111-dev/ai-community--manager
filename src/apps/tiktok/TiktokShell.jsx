...
export default function TiktokShell({ counts, outletContext }) {
  ...
  const [animateEntrance, setAnimateEntrance] = useState(Boolean(location.state?.animateTopbar));
  /** Aperçu mobile / bureau : via la fenêtre du navigateur uniquement (plus de barre Ordinateur/Mobile). */
  const tiktokPlatformLayout = 'desktop';

  useEffect(() => {
    if (!location.state?.animateTopbar) return undefined;
    setAnimateEntrance(true);
    const timer = window.setTimeout(() => setAnimateEntrance(false), 520);
